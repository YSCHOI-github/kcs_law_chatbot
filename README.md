...
- [🚀 1인1봇 프로젝트](#-1인1봇-프로젝트)
- [🌟 이 챗봇의 3가지 특징](#-이-챗봇의-3가지-특징)
  - [1. 무료: 구글 제미나이 무료 모델 사용](#1-무료-구글-제미나이-무료-모델-사용)
  - [2. 멀티 에이전트: 여러 AI의 협업으로 성능 극대화](#2-멀티-에이전트-여러-ai의-협업으로-성능-극대화)
  - [3. 하이브리드 RAG (AI + TF-IDF): 일반 노트북에서도 빠르게 구동](#3-하이브리드-rag-ai--tf-idf-일반-노트북에서도-빠르게-구동)
- [📚 데이터 - 5개 법령 패키지 + 사용자 추가 업로드 가능](#-데이터---5개-법령-패키지--사용자-추가-업로드-가능)
  - [📦 5개 법령 패키지 (총 43개 법령 파일)](#-5개-법령-패키지-총-43개-법령-파일)
  - [📂 사용자 법령 업로드 기능](#-사용자-법령-업로드-기능)
  - [🧩 데이터 구조](#-데이터-구조)
- [💬 사용법](#-사용법)
  - [🤖 질의 응답 기능 (AI 챗봇 탭)](#-질의-응답-기능-ai-챗봇-탭)
  - [🔍 법령 검색 기능 (법령 검색 탭)](#-법령-검색-기능-법령-검색-탭)
- [⚠️ 한계](#️-한계)
- [🧱 프로젝트 구조](#-프로젝트-구조)
- [🧠 기술 아키텍처](#-기술-아키텍처)
...
- **AI 답변 출력 과정**:
  - 사용자 질문에 대해 **각 법령 AI**의 답변을 **Expander**(박스)에 출력
  - 최종 답변 완성 시 각 법령의 답변이 있는 Expander가 닫히고, **HEAD AI**의 최종 답변 출력
...
사용자는 다음 3가지 방법 중 하나를 선택하여 법령 데이터를 로드할 수 있습니다:

#### 📂 사전 패키지 사용 (권장)
- **설명**: 미리 다운로드된 5개 패키지 중 선택
- **장점**: 즉시 사용 가능, 안정적
- **사용법**: 메인 화면에서 패키지 버튼 클릭

#### 📤 사용자 파일 업로드
- **설명**: PDF/TXT 파일 직접 업로드
- **장점**: 내부 규정, 미등록 법령 등 자유롭게 추가 가능
- **지원 형식**: PDF, TXT

#### 🌐 법령 API 다운로드 (최신 법령)
- **설명**: 법제처 API를 통해 실시간으로 법령명 입력하여 다운로드
- **장점**: 최신 법령 즉시 다운로드, 3단비교 자동 생성
- **필요사항**: LAW_API_KEY 발급 필요 (법률 및 행정규칙 모두 동일한 키 사용)
...
**1) 사전 다운로드된 패키지 선택**
- 상단의 "📂 사전 패키지 사용" 라디오 버튼 선택
- 패키지 버튼 클릭 (예: "관세조사")
- 자동으로 TF-IDF 벡터화되어 즉시 챗봇 사용 가능

**2) 사용자 파일 업로드 (PDF, txt)**
- 상단의 "📤 사용자 파일 업로드" 라디오 버튼 선택
- "Browse files" 클릭하여 파일 선택 (여러 파일 동시 선택 가능)
- "📥 업로드 및 처리" 버튼 클릭
- 자동으로 TF-IDF 벡터화되어 챗봇 사용 가능
- 사이드바 "업로드/다운로드된 법령 관리"에 업로드된 파일명, 조문 개수 표시
- JSON 다운로드, 삭제 기능 제공

**3) 법령 API 실시간 다운로드**

*사전 준비*
- `.env` 파일에 API 키 설정: `LAW_API_KEY` (법률 및 행정규칙 모두 사용)

*사용 순서*
- 상단의 "🌐 법령 API 다운로드" 라디오 버튼 선택

- **법률 다운로드 (좌측 패널)**
  - 법령명 입력 (콤마로 구분, 예: `관세법, 관세법 시행령, 관세법 시행규칙`)
  - "📥 법률 다운로드" 버튼 클릭
  - 법/법률로 끝나는 법령은 3단비교 자동 다운로드

- **행정규칙 다운로드 (우측 패널)**
  - 행정규칙명 입력 (콤마로 구분, 예: `관세평가 운영에 관한 고시, 관세조사 운영에 관한 훈령`)
  - "📥 행정규칙 다운로드" 버튼 클릭

- **데이터 변환**
  - 하단의 "🔄 다운로드된 법령 데이터 변환" 버튼 클릭
  - TF-IDF 벡터화 완료 후 챗봇 사용 가능
  - 사이드바에서 다운로드한 법령을 JSON으로 저장 가능
...
2. **법령 API Key** (선택 - API 다운로드 모드 사용 시 필수):
   - 발급처: [법제처 오픈API](https://open.law.go.kr/LSO/openApi/cuAskList.do)
   - 용도: 법률, 시행령, 시행규칙 실시간 다운로드
   - 비용: 무료

3. **행정규칙 API Key** (선택 - API 다운로드 모드 사용 시 필수):
   - 발급처: [법제처 오픈API](https://open.law.go.kr/LSO/openApi/cuAskList.do)
   - 용도: 고시, 훈령, 예규 등 실시간 다운로드
   - 비용: 무료

**참고**: 사전 패키지 모드나 사용자 업로드 모드만 사용할 경우, Google API Key만 있으면 됩니다.
...
LAW_API_KEY=여기에_법령_API_키_입력 (선택 - 법률 및 행정규칙 모두 사용)
```
