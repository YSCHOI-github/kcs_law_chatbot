...
**"최신 법령을 실시간으로 다운로드합니다"**

#### 사전 준비
1. `.env` 파일에 API 키 설정:
   - `LAW_API_KEY`: 법률, 시행령, 시행규칙, 행정규칙(고시, 훈령, 예규 등) 모두 다운로드용

#### 사용 순서

**1. 법률 다운로드 (좌측 패널)**
   - 법령명 입력 (콤마로 구분)
     - 예시: `관세법, 관세법 시행령, 관세법 시행규칙`
   - "📥 법률 다운로드" 버튼 클릭
   - **자동 처리**: 법/법률로 끝나는 법령은 3단비교 자동 다운로드
     - 예: "관세법" 입력 시 → "관세법" + "관세법_3단비교" 자동 생성

**2. 행정규칙 다운로드 (우측 패널)**
   - 행정규칙명 입력 (콤마로 구분)
     - 예시: `관세평가 운영에 관한 고시, 관세조사 운영에 관한 훈령`
   - "📥 행정규칙 다운로드" 버튼 클릭
...
#### 💡 활용 팁
- **법령명 정확도**: 법제처 공식 법령명 사용 권장
- **3단비교 자동**: "법" 또는 "법률"로 끝나는 법령만 자동 생성
- **JSON 저장**: 사이드바에서 다운로드한 법령을 JSON으로 저장 가능
